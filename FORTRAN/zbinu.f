...
C   ZR IS THE REAL PART OF THE ARGUMENT
C   ZI IS THE IMAGINARY PART OF THE ARGUMENT
C   FNU IS THE ORDER
C   KODE IS A SCALING OPTION
C   N IS NUMBER OF TERMS IN SEQUENCE

C   RETURNS
C    CYR(Real part), CYI(Imag Part) , NZ(Underflow count)
...
C***FIRST EXECUTABLE STATEMENT  ZBINU
      ...
      NN = N
      DFNU = FNU + (N-1)
C   IF THE ABS VALUE IS <=2 DO A POWER SERIES
      IF (AZ.LE.2.0D0) GO TO 10 
C   IF .25*AZ**2 > FNU+N DO NOT POWER SERIES
